{{-- themes/obsidian/views/cart.blade.php --}}

@php
    use Illuminate\Support\Facades\DB;

    // Load Cart Editor config (same pattern as Terms extension)
            ...
            @if($headerEnabled)
                <div class="mb-8">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            <h1 class="text-3xl md:text-4xl font-semibold tracking-tight text-primary-100">
                                {{ $headerTitle }}
                            </h1>
                        ...
                        @if($stepperEnabled)
                            <div class="flex items-center gap-3 text-sm">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="text-primary-100 font-medium">{{ $step1Label }}</div>
                                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="rounded-3xl border border-neutral bg-background-secondary/40 p-6 md:p-7">
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <div class="min-w-0 flex items-start gap-4">
                                    {{-- Icon --}}
                                    <div class="w-12 h-12 rounded-2xl bg-primary/15 border border-primary/25 flex items-center justify-center shrink-0">
                                    ...
                                    <div class="min-w-0">
                                        <div class="text-lg md:text-xl font-semibold text-primary-100 truncate">
                                            {{ $item->product->name }}
                                        </div>
                                ...
                                <div class="shrink-0 text-right">
                                    <div class="text-2xl md:text-3xl font-semibold text-primary-100">
                                    ...
                                    @if ($item->quantity > 1)
                                        <div class="mt-1 text-xs text-primary-500/80">
                                            {{ $item->price }} each
                                        </div>
                                    @endif
                                </div>
                                        ...
                                        <div class="flex items-start gap-3">
                                            <div class="mt-1 w-7 h-7 rounded-xl bg-background-secondary/60 border border-neutral flex items-center justify-center shrink-0">
                                                <svg class="w-4 h-4 text-primary-500" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0ll4-4z" clip-rule="evenodd" />
                                                </svg>
                                            </div>
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <div class="text-sm font-medium text-primary-100 truncate">
                                                    {{ $option['value_name'] }}
                                                </div>
                                            </div>
                                        </div>
                            ...
                            {{-- Actions + quantity --}}
                            <div class="mt-6 pt-6 border-t border-neutral/60 space-y-4">
                    ...
                    @if (Cart::items()->count() > 0 && $continueEnabled)
                        <div>
                            <a href="{{ url()->previous() }}" class="text-sm text-primary-500 hover:text-primary-100 inline-flex items-center gap-2">
                ...
                <div class="lg:col-span-1 space-y-6">
                    @if (Cart::items()->count() > 0 && $summaryEnabled)
                        <div class="rounded-3xl border border-neutral bg-background-secondary/40 p-6 lg:sticky lg:top-24">
                            <h2 class="text-base font-semibold text-primary-100 mb-5">
                                {{ $summaryTitle }}
                            </h2>

                            {{-- Coupon --}}
                            <div class="mb-6">
                                <div class="text-sm text-primary-500 mb-2">{{ $couponLabel }}</div>
                                ...
                                @else
                                    <div class="flex items-center justify-between gap-2 bg-background-secondary/30 border border-neutral rounded-2xl px-4 py-3">
                                        <div class="text-sm text-primary-100 font-semibold truncate">
                                            {{ $coupon->code }}
                                        </div>
                            ...
                            <div class="space-y-3 border-t border-neutral/60 pt-5 text-sm">
                                ...
                                <div class="pt-4 mt-1 border-t border-neutral/60 flex justify-between items-baseline">
                                    <span class="text-primary-100 font-semibold">{{ __('invoices.total') }}</span>
                                    <span class="text-2xl font-semibold text-primary-100">
                            ...
                            @if(config('settings.tos'))
                                <div class="mt-5">
                                    <label for="cart_tos" class="block w-full rounded-2xl border border-neutral bg-background-secondary/30 px-4 py-3 cursor-pointer">
                                        <div class="flex items-start gap-3">
                                            <input
                                                id="cart_tos"
                                                type="checkbox"
                                            ...
                                            <div class="min-w-0 text-sm text-primary-100/80 leading-5">
                            ...
                            {{-- Trust bullets --}}
                            @if($trustEnabled)
                                <div class="mt-5 space-y-2 text-sm text-primary-500">
                        ...
                        @if($whyEnabled)
                            <div class="rounded-3xl border border-neutral bg-background-secondary/40 p-6">
                                <h3 class="text-base font-semibold text-primary-100 mb-4">{{ $whyTitle }}</h3>
                                    ...
                                    @foreach($whyItems as $why)
                                        ...
                                        @if($title !== '')
                                            <div class="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <div class="font-medium text-primary-100">{{ $title }}</div>
                                                    @if($sub !== '')
                                                        <div class="text-primary-500">{{ $sub }}</div>
                                                    @endif
                                                </div>
                                            </div>
                                        @endif
                        ...
                        @if($helpEnabled)
                            <div class="rounded-3xl border border-primary/25 bg-primary/5 p-6">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="min-w-0">
                                        <div class="text-base font-semibold text-primary-100">{{ $helpTitle }}</div>
                                        <div class="mt-1 text-sm text-primary-500">
                                            {{ $helpSubtitle }}
                                        </div>
